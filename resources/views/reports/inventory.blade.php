    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('reports.inventory', ['export' => 'pdf'] + request()->all()) }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:ring ring-red-300 disabled:opacity-25 transition ease-in-out duration-150">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <!-- Filters -->
                    <form action="{{ route('reports.inventory') }}" method="GET" class="mb-6">
                        <div class="flex flex-wrap gap-4">
                            ...
                            <div class="flex items-end">
                                ...
                                @if(request()->anyFilled(['category_id', 'status', 'stock_status']))
                                    <a href="{{ route('reports.inventory') }}" class="ml-2 inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                        Clear
                                    </a>
                                @endif
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        <div class="bg-gray-50 rounded-lg p-4">
                            ...
                            <p class="text-3xl font-bold text-indigo-600">{{ $products->count() }}</p>
                        </div>
                        
                        <div class="bg-gray-50 rounded-lg p-4">
                            <h3 class="text-lg font-medium text-gray-900 mb-2">Total Stock Value</h3>
                            <p class="text-3xl font-bold text-green-600">${{ number_format($products->sum(function($product) { return $product->price * $product->quantity; }), 2) }}</p>
                        </div>
                        
                        <div class="bg-gray-50 rounded-lg p-4">
                            <h3 class="text-lg font-medium text-gray-900 mb-2">Low Stock Items</h3>
                            <p class="text-3xl font-bold text-red-600">{{ $products->where('quantity', '<', 10)->count() }}</p>
                        </div>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($products as $product)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">${{ number_format($product->price * $product->quantity, 2) }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
